  ...
  ExpectJs.(
    () => {
      ...
      test(
        "with light theme",
        () => {
          let component = TestHelpers.setup(<ComponentPlayground theme=Light />);
          expect(component) |> toMatchSnapshot
        }
      );
      test(
        "with preview background color",
        () => {
          let component =
            TestHelpers.setup(<ComponentPlayground previewBackgroundColor="#7315FF" />);
          expect(component) |> toMatchSnapshot
        }
      );
      test(
        ...
        () => {
          let component =
            TestHelpers.setup(<ComponentPlayground scope={"logger": Js.Console.log} />);
          expect(component) |> toMatchSnapshot
        }
      );
      test(
        "with code",
        () => {
          let component = TestHelpers.setup(<ComponentPlayground code="test" />);
          expect(component) |> toMatchSnapshot
        }
      )
    }
